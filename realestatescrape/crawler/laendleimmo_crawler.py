...
class LaendleimmoScraper(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for url in urls:
            yield scrapy.Request(url=url, callback=self.parse, headers={"User-Agent": random.choice(USER_AGENTS)})
    ...
    def parse(self, response):
        ...
        for next_page in response.css("ul li.next a"):
            yield response.follow(next_page, self.parse, headers={"User-Agent": random.choice(USER_AGENTS)})
